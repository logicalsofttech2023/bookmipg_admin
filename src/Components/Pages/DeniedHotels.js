...
const DeniedHotels = () => {
  ...
  let getHotelData = () => {
    ...
    axios
      .get(
        `${process.env.REACT_APP_BASE_URL}api/admin/getAllHotelsByInAdmin?page=${activePage}&limit=${itemsPerPage}&search=${searchValue}&adminVerify=false`,
        {
          headers: {
      ...
      .then((res) => {
        ...
        setAdminVerify(res?.data?.hotels?.adminVerify);
      })
      .catch((error) => {
